...
async function parserFunction(dirPath: string): Promise<void> {
  ...
  try {
    const files: string[] = await fsPromises.readdir(csvDirectory);
    const csvFiles: string[] = files.filter((file) => path.extname(file) === '.csv');
    ...
    for (const csvFile of csvFiles) {
      ...
      const jsonData: unknown = await parseCSVFile(csvFilePath);
      const jsonFilePath: string = path.join(pathToConvert, `${path.parse(csvFile).name}.json`);
...
async function writeJSONFile(filePath: string, jsonData: unknown) {
